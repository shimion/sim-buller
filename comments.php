...
<!-- You can start editing here. -->
	<?php 
		...
		if($no_comments == 0){
			if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { $comment_0 = pmc_stripText($pmc_data['translation_comment_no_responce']); } else {  $comment_0 = __('No Responses','wp-buler'); } 
		}
		else if($no_comments == 1){
			if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { $comment_1 = pmc_stripText($pmc_data['translation_comment_one_comment']); } else {  $comment_1 = __('One Response','wp-buler'); } 
		}
		else{
			if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { $comment_2 = pmc_stripText($pmc_data['translation_comment_max_comment']); } else {  $comment_2 = __('Responses','wp-buler'); } 
		}
...
<?php $aria_req = ''; $post_id = ''; $defaults = array( 'fields' => apply_filters( 'comment_form_default_fields', array(
    ...
    'comment_field' => '' .
                ...
                '</div>',
    'must_log_in' => '
<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ,'wp-buler'), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>

',
    'logged_in_as' => '
<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%s">%s</a>. <a title="Log out of this account" href="%s">Log out?</a></p>

' ,'wp-buler'), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ),
